...
internal class CheckInSuiteMenuItem : IMenuItem<User>
{
    public static CheckInSuiteMenuItem Singleton { get; } = new();

    public string GetTitle(ApplicationContext application, User? user)
    {
        return user == null ? throw new Exception("user is null") : "入住套房";
    }
    ...
    public (IMenu?, object?) Run(ApplicationContext application, User? user)
    {
        ...
        try
        {
            Room[] rooms = application.RoomService.GetNotCheckInRooms(0, RoomType.Suite, 5);
            if (rooms.Length == 0)
            {
            ...
            while (true)
            {
                ...
                switch (key.Key)
                {
                    ...
                    {
                        Room[] prev = application.RoomService.GetNotCheckInRooms(rooms[0].Id, RoomType.Suite, -5);
                    ...
                    {
                        Room[] next = application.RoomService.GetNotCheckInRooms(rooms[^1].Id, RoomType.Suite, 5);
