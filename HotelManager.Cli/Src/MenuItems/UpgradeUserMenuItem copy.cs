...
internal class DowngradeUserMenuItem : IMenuItem<User>
{
    public static readonly DowngradeUserMenuItem Singleton  = new();
            ...
            while (true)
            {
                ...
                switch (key.Key)
                {
                    ...
                    case ConsoleKey.Enter:
                    {
                        ...
                        Console.Write($"| 确定要降级 {users[selected].Name} (y): ");
                        if (Console.ReadLine() != "y") break;

                        application.UserService.DowngradeUser(users[selected].Name);
                        return (UserManagerMenu.Singleton, context);
                    }
                }
            }
