...
internal class RemoveUserMenuItem : IMenuItem<User>
{
    public static readonly RemoveUserMenuItem Singleton  = new();
            ...
            while (true)
            {
                ...
                switch (key.Key)
                {
                    ...
                    case ConsoleKey.Enter:
                    {
                        ...
                        Console.Write($"| 确定要删除 {users[selected].Name} (y): ");
                        if (Console.ReadLine() != "y") break;

                        application.UserService.RemoveUser(users[selected].Name);
                        return (UserManagerMenu.Singleton, context);
                    }
                }
            }
