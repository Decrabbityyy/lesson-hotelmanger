...
internal class CheckInDoubleMenuItem : IMenuItem<User>
{
    public static CheckInDoubleMenuItem Singleton { get; } = new();

    public string GetTitle(ApplicationContext application, User? user)
    {
        return user == null ? throw new Exception("user is null") : "入住双人间";
    }
    ...
    public (IMenu?, object?) Run(ApplicationContext application, User? user)
    {
        ...
        try
        {
            Room[] rooms = application.RoomService.GetNotCheckInRooms(0, RoomType.Double, 5);
            if (rooms.Length == 0)
            {
            ...
            while (true)
            {
                ...
                switch (key.Key)
                {
                    ...
                    {
                        Room[] prev = application.RoomService.GetNotCheckInRooms(rooms[0].Id, RoomType.Double, -5);
                    ...
                    {
                        Room[] next = application.RoomService.GetNotCheckInRooms(rooms[^1].Id, RoomType.Double, 5);
